...
/**
 * WASD Rig component to control entities using WASD keys.
 * The previous WASD component presumed this.el was the camera.
 * This will presume the camera is a child of this rig entity.
 */
AFRAME.registerComponent("rig-wasd-controls", {
  ...
  init: function () {
    ...
    this.childEntityWithCameraComponent = this.getChildCamera();
  },
    ...
    for (let i = 0; i < rig.children.length; i++) {
      if (rig.children[i].isEntity) {
        ...
        let hasCamera = Boolean(
          Object.keys(child.components).filter((c) => c === "camera").length
        );
        if (hasCamera) {
          return child;
        }
  ...
  tick: function (time, delta) {
    ...
    rig.object3D.position.add(this.getMovementVector()(delta));
  },
  ...
  getMovementVector: function () {
    var head = this.childEntityWithCameraComponent;
    var directionVector = new THREE.Vector3(0, 0, 0);
    var rotationEuler = new THREE.Euler(0, 0, 0, "YXZ");
    const self = this;

    return function (delta) {
      var rotation = head.getAttribute("rotation");
      ...
      xRotation = self.data.fly ? rotation.x : 0;
...
// Put rig component and a-rig primitive below.
